...
namespace ice
{
    ...
    template<class... Args>
    void log_info(std::string_view message, Args const&... args) noexcept
    {
        detail::log_info(std::move(message), fmt::make_format_args(args...));
    }

    template<class... Args>
    void log_verbose(std::string_view message, Args const&... args) noexcept
    {
        detail::log_verbose(std::move(message), fmt::make_format_args(args...));
    }

    template<class... Args>
    void log_warning(std::string_view message, Args const&... args) noexcept
    {
        detail::log_warning(std::move(message), fmt::make_format_args(args...));
    }

    template<class... Args>
    void log_warning_origin(std::string_view file, int32_t file_line, std::string_view message, Args const&... args) noexcept
    {
        detail::log_warning_origin(std::move(file), file_line, std::move(message), fmt::make_format_args(args...));
    }

    template<class... Args>
    void log_error(std::string_view message, Args const&... args) noexcept
    {
        detail::log_error(std::move(message), fmt::make_format_args(args...));
    }

    template<class... Args>
    void log_error_origin(std::string_view file, int32_t file_line, std::string_view message, Args const&... args) noexcept
    {
        detail::log_error_origin(std::move(file), file_line, std::move(message), fmt::make_format_args(args...));
    }

    template<class... Args>
    void log_crash(std::string_view message, Args const&... args) noexcept
    {
