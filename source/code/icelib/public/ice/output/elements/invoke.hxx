...
namespace ice::output
{
    ...
    template<>
    auto identifier(Invoke const& data) noexcept -> std::string;

    template<>
    auto identifier(Initializer const& data) noexcept -> std::string;
    ...
    protected:
        ...
        virtual auto get_element(std::string dataid) const noexcept -> ASTElement* = 0;
    ...
    template<class T>
    auto ASTInvokeable::set_argument(std::string name, T const& element_data, bool new_line /*= false*/) noexcept -> auto&
    {
    ...
    template<class T>
    auto ASTInvokeable::chain_element(T const& element_data) noexcept -> auto&
    {
        using return_type_raw = decltype(factory().create(element_data));
        using return_type = typename std::decay_t<return_type_raw>::element_type;
    ...
    template<class T>
    auto ASTInvokeable::append_element(T const& element_data) noexcept -> auto&
    {
        using return_type_raw = decltype(factory().create(element_data));
        using return_type = typename std::decay_t<return_type_raw>::element_type;
